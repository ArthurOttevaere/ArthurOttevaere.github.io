...
//      favicon.svg            modern SVG favicon (ink tile, the "a." mark)
...
//  900, with the round orange dot) — see Icon.Logo in src/icons.jsx.
//
//  HOW TO RUN
//      npm run icons
// =============================================================================
...
const INK = '#101014', PAPER = '#F6F4EF', ACCENT = '#FF4A12';
...
const MARK =
  `<g transform="translate(8,${(100 - 552 * S) / 2}) scale(${S.toFixed(5)})">` +
    ...
    `<circle cx="963" cy="405" r="135" fill="${ACCENT}"/>` +
  `</g>`;

const tile = rx =>
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">` +
  `<rect width="100" height="100" rx="${rx}" fill="${INK}"/>${MARK}</svg>`;
